...
export const Header = () => {
  ...
  useEffect(() => {
    ...
    const headerOffset = document.querySelector('header')?.getBoundingClientRect().top
  ...
  return (
    <header
      ...
      <div id="header" className="w-[1200px] m-auto flex items-center">
        <Image src={'/kmong-main-logo.gif'} width={100} height={60} alt="main-logo" />
                  ...
                  <ul>
                    {recentSearches.length > 0 ? (
                      recentSearches.map((item) => (
                        <li key={item} className="py-1 cursor-pointer hover:bg-gray-100">
                          {item}
                        </li>
                      ))
                    ) : (
                      <p className="text-center text-gray-400">최근 검색어가 없습니다.</p>
                    )}
                  </ul>
                </div>
                <div className="p-5">
        ...
        <div className="flex w-[40%]">
          <div className="mr-4">
            <Link href={'/#'}>
              엔터프라이즈{' '}
              <span className="rounded-xl bg-[#ebf4ff] px-1 py-1 text-blue-500 text-xs font-extrabold">기업용</span>
            </Link>
          </div>
          <div className="mr-4">
